...
EasyCore.MongoDbRepository 提供了一个实体基类 EasyCoreEntity，包含并发标记/软删除/以及多租户ID。
...
EasyCore.MongoDbRepository 提供了一个仓储基类 MongoDbRepository<TDbContext, TEntity> 和仓储抽象 IRepository<TEntity>。
...
EasyCore.MongoDbRepository提供一个IQueryable<T>.WhereIf的支持。
...
EasyCore.MongoDbRepository就可以自动实现实体的CURD操作。
